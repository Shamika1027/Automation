...
def ssh_to_core(ip, user, password):
    ...
    try:
        ...
        print("dHCP bindings saved to bindings.txt")
        return output
...
def print_ip_and_mac():
    ...
    with open('bindings.txt', 'r') as file:
        arp_table = file.read()
...
def extract_bindings():
    ...
    while i < len(lines):
        line = lines[i].strip()
        if line and not line.startswith("Bindings from"):
            # Check if the line contains an IP address (assuming it's in IPv4 format)
            if len(line.split('.')) == 4:
                ip_address = line.split()[0]
                client_id = lines[i + 2].strip()  # Client ID is located two lines below IP address
                bindings.append((ip_address, client_id))
        i += 1
...
def get_host_name():
    ...
    for index, row in mac_df.iterrows():
        ...
        if pd.notnull(ip_address):
            ip_host_array.append([ip_address, host_name])
...
def main():
    ...
    # Get Ip 
    ip_mac_addresses = extract_bindings()
    if ip_mac_addresses:
        for binding in ip_mac_addresses:
            print(binding)
    else:
        print("no ip")  
